...
class StateExtractor:
    ...
    def extract(
        ...
        if include_ocr:
            ...
            state.level = self._read_level(image_bgr, regions.get("level"), base_size, debug_dir)
    ...
    def save_debug_crops(self, image_bgr: np.ndarray, debug_dir: Path) -> None:
        ...
        for key, region in regions.items():
            if isinstance(region, dict) and "bbox" in region:
                ...
                save_crop(debug_dir / f"{key}.png", crop(image_bgr, bbox))
                self._draw_debug_bbox(overlay, bbox, key)
            elif isinstance(region, list):
                for idx, entry in enumerate(region):
                    if isinstance(entry, dict) and "bbox" in entry:
                        bbox = scaled_bbox(entry["bbox"], base_size, image_bgr.shape)
                        save_crop(debug_dir / f"{key}_{idx:02d}.png", crop(image_bgr, bbox))
                        self._draw_debug_bbox(overlay, bbox, f"{key}_{idx}", list_region=True)
        save_crop(debug_dir / "regions_overlay.png", overlay)

    @staticmethod
    def _draw_debug_bbox(overlay: np.ndarray, bbox: list[int], label: str, list_region: bool = False) -> None:
        x, y, w, h = bbox
        color = (70, 220, 255) if not list_region else (90, 180, 90)
    ...
    def _read_level(self, image_bgr: np.ndarray, region: dict | None, base_size: list[int], debug_dir: Path | None) -> FieldValue:
        if not region:
            return FieldValue(source="level")
        image = crop(image_bgr, scaled_bbox(region["bbox"], base_size, image_bgr.shape))
        if debug_dir:
            save_crop(debug_dir / "ocr_level.png", image)
        text_result = self.ocr.read_text(image)
        digit_result = self.ocr.read_text(image, "0123456789")
        value, confidence = parse_int(digit_result, 1, 10)
        if value is None:
            value, confidence = parse_int(text_result, 1, 10)
        raw = text_result.text or digit_result.text
        return FieldValue(value=value, confidence=round(confidence, 3), source="level", raw_text=raw)
